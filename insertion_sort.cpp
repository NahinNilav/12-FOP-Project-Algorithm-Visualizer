    ...
    while (new_window.isOpen())
    {
        ...
        else if (part == 2 && !is_sorted && once_pressed_sort_key)
        {
            for (int i = 0; i < set_size; i++)
            {
                ...
                rectangle_array[i].setRotation(180);
                if (i == pos1 || i == pos2)
                    rectangle_array[i].setFillColor(Color::White);
                else
                    rectangle_array[i].setFillColor(get_colour_by_length(heights[i], rectange_colour));
                new_window.draw(rectangle_array[i]);
                position_of_rectangle += width_of_rectangle + 1;
            ...
            // it_time++;
            if (it_pos > 0 && heights[it_pos] < heights[it_pos - 1])
            {
                clock.restart();
            ...
            {
                part = 3;
                position_of_iterator = 1.f + ((width_of_rectangle + 1.f) * (insertion_bound + 1) - width_of_rectangle / 2);
                it_pos = insertion_bound;
                insertion_bound++;
            }
        }
        else if (part == 3 && !is_sorted && once_pressed_sort_key)
        {
            ...
            if (is_sorted)
            {
                it_pos = 0;
                insertion_bound = 1;
                position_of_iterator = width_of_rectangle / 2 + 1.f;
            }
        }
    here:
        new_window.display();
